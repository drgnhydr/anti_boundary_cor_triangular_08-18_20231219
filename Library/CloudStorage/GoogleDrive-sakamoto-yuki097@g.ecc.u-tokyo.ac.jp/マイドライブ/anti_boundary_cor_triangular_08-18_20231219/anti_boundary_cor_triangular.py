...
class Game():
    def __init__(self, L, r, K, w, adj_list):
        self.L = L
        self.r = r
        self.K = K
        self.w = w
        ...
        self._payoff_matrix = np.array([[1, 0], [r, 0]])
    ...
    def append_df(self, df, iteration, step):
        ...
        record = pd.Series([self.L, self.r, self.K, self.w, iteration, step, self.ratio(), self.ave_payoff(), dt_now], index=cols)
        df = df.append(record, ignore_index=True)
        return df

    def display(self, iteration, step):
        print("L", self.L, "r:", self.r, "K:", self.K, "w", self.w, "iter:", iteration, "step:", (step+1) // (self.N),
            "ratio:", self.ratio(), "ave_payoff:", self.ave_payoff())
    
def call_cols():
    return ["L", "r", "K", "w", "iterations", "steps", "ratio", "ave_payoff", "datetime"]
...
def main():
    ...
    G = nx.triangular_lattice_graph(L, L, periodic=True, with_positions=True, create_using=None)
    G = nx.convert_node_labels_to_integers(G)
    adj_list = nx.to_dict_of_lists(G)

    cols = call_cols()
    #df = pd.DataFrame(index=[], columns=cols)
    df_correlation = pd.DataFrame(index=[], columns=cols)
    for K in K_list:
        for iteration in range(iterations):
            game = Game(L, r, K, w, adj_list)
            flag_break = False
            game.display(iteration, step=0)
            # df = game.append_df(df, iteration, step=0)
            
            state_array = [game.state.copy()]
            for step in range(total_steps * game.N):
                ...
                if (step+1) % (print_steps*game.N) == 0:
                    ...
                    state_array.append(game.state.copy())
                    if game.state.sum() in [0, game.N]:
                        flag_break = True
                        break
            state_array = np.array(state_array)
            braket_x_array = state_array.mean(axis=0)
            correlation_array = []
            for n in range(game.N):
                for m in game.adj_list[n]:
                    braket_xy_nm = np.mean(state_array[:,n] * state_array[:,m])
                    correlation_nm = braket_xy_nm - braket_x_array[n] * braket_x_array[m]
                    correlation_array.append(correlation_nm)
            correlation_mean = 0 if flag_break else np.mean(correlation_array[len(correlation_array)//2:])
            dt_now = datetime.datetime.now().strftime('%Y-%m-%d-%H-%M-%S')
            df_correlation_iter = pd.DataFrame({
                "L": game.L,
                "r": game.r,
                "K": game.K,
                "w": game.w,
            ...
            df_correlation=df_correlation.append(df_correlation_iter,ignore_index=True)
    dt_now = datetime.datetime.now().strftime('%Y-%m-%d-%H-%M-%S')
    file_correlation = "correlation_" + str(dt_now) + "_L" + str(L) + "_w" + str(w) + "_r" + str(r) + "_triangular" + ".csv"
    df_correlation.to_csv(file_correlation)
    
    # file_name = str(dt_now) + "_L" + str(L) + "_w" + str(w) + "_r" + str(r) + "_square" + ".csv"
    # df.to_csv(file_name)
